...
const constexpr Months CronData::months_with_31[NUMBER_OF_LONG_MONTHS] = {
    Months::January, Months::March, Months::May, Months::July, Months::August, Months::October, Months::December};

const std::vector<std::string> CronData::month_names{"JAN", "FEB", "MAR", "APR", "MAY", "JUN",
...
std::vector<std::string> CronData::split(const std::string& s, char token) {
    ...
    std::copy(std::sregex_token_iterator(s.begin(), s.end(), splitter, -1), std::sregex_token_iterator(),
...
bool CronData::check_dom_vs_dow(const std::string& dom, const std::string& dow) const {
    ...
    auto check = [](const std::string& l, std::string r) { return l == "*" && (r != "*" || r == "?"); };
