...
SCENARIO("Adding a task") {
    GIVEN("A Cron instance with no task") {
        ...
        THEN("Starts with no task") { REQUIRE(c.count() == 0); }

        WHEN("Adding a task that runs every second") {
            REQUIRE(c.add_schedule("A task", "* * * * * ?", [&expired](auto&) { expired = true; }));
...
SCENARIO("Get delay using Task-Information") {
    ...
    GIVEN("A Cron instance with one task expiring in  2 seconds, but taking 3 seconds to execute") {
        ...
        Cron<> c;
        REQUIRE(c.add_schedule("Two", "*/2 * * * * ?", [&_2_second_expired, &_delay](auto& i) {
            _2_second_expired++;
            _delay = i.get_delay();
            std::this_thread::sleep_for(3s);
        }));
        ...
        WHEN("Exactly schedule task") {
            while (_2_second_expired == 0) c.tick();
...
SCENARIO("Task priority") {
    GIVEN("A Cron instance with two tasks expiring in 3 and 5 seconds, added in 'reverse' order") {
        ...
        REQUIRE(c.add_schedule("Five",
                               ...
                               [&_5_second_expired](auto&) { _5_second_expired++; }));

        REQUIRE(c.add_schedule("Three",
                               create_schedule_expiring_in(c.get_clock().now(), hours{0}, minutes{0}, seconds{3}),
                               [&_3_second_expired](auto&) { _3_second_expired++; }));
...
class TestClock : public ICronClock {
public:
    std::chrono::system_clock::time_point now() const override { return current_time; }

    std::chrono::seconds utc_offset(std::chrono::system_clock::time_point) const override { return 0s; }

    void add(system_clock::duration time) { current_time += time; }

    void set(system_clock::time_point new_time) { current_time = new_time; }
...
SCENARIO("Clock changes") {
    GIVEN("A Cron instance with a single task expiring every hour") {
        ...
        // Midnight
        clock.set(sys_days{2018y / 05 / 05});
...
SCENARIO("Multiple ticks per second") {
    ...
    // Every 10 seconds
    REQUIRE(c.add_schedule("Clock change task", "*/10 0 * * * ?", [&run_count](auto&) { run_count++; }));
...
SCENARIO("Tasks can be added and removed from the scheduler") {
    GIVEN("A Cron instance with no task") {
        ...
        WHEN("Adding 5 tasks that runs every second") {
            REQUIRE(c.add_schedule("Task-1", "* * * * * ?", [&expired](auto&) { expired = true; }));

            REQUIRE(c.add_schedule("Task-2", "* * * * * ?", [&expired](auto&) { expired = true; }));

            REQUIRE(c.add_schedule("Task-3", "* * * * * ?", [&expired](auto&) { expired = true; }));

            REQUIRE(c.add_schedule("Task-4", "* * * * * ?", [&expired](auto&) { expired = true; }));

            REQUIRE(c.add_schedule("Task-5", "* * * * * ?", [&expired](auto&) { expired = true; }));

            THEN("Count is 5") { REQUIRE(c.count() == 5); }
