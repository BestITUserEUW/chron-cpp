...
public:
    ...
    static const Months months_with_31[NUMBER_OF_LONG_MONTHS];
    ...
    const std::set<Minutes>& get_minutes() const { return minutes; }

    const std::set<Hours>& get_hours() const { return hours; }

    const std::set<DayOfMonth>& get_day_of_month() const { return day_of_month; }

    const std::set<Months>& get_months() const { return months; }

    const std::set<DayOfWeek>& get_day_of_week() const { return day_of_week; }
...
private:
    ...
    template <typename T>
    bool validate_literal(const std::string& s, std::set<T>& numbers, const std::vector<std::string>& names);
...
template <typename T>
bool CronData::validate_literal(const std::string& s, std::set<T>& numbers, const std::vector<std::string>& names) {
    std::vector<std::string> parts = split(s, ',');
