...
public:
    ...
    std::chrono::system_clock::duration get_delay() const override { return delay; }
    ...
    bool operator>(const Task &other) const { return next_schedule > other.next_schedule; }

    bool operator<(const Task &other) const { return next_schedule < other.next_schedule; }

    bool is_expired(std::chrono::system_clock::time_point now) const;

    std::chrono::system_clock::duration time_until_expiry(std::chrono::system_clock::time_point now) const;
...
inline bool operator==(const std::string &lhs, const Task &rhs) { return lhs == rhs.get_name(); }

inline bool operator==(const Task &lhs, const std::string &rhs) { return lhs.get_name() == rhs; }

inline bool operator!=(const std::string &lhs, const Task &rhs) { return !(lhs == rhs); }

inline bool operator!=(const Task &lhs, const std::string &rhs) { return !(lhs == rhs); }
